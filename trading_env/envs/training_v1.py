...
class trading_env:
    ...
    def reset(self):
        ...
        if self.normalize_price:
            avg = np.average(self.price)
            self.price = self.price / avg * 100.0
            self.obs_features[:, :-1] = self.obs_features[:, :-1] / avg * 100.0
    ...
    def _short_cover(self, current_price_mean, current_mkt_position):
        ...
        self.chg_reward[:] = (
            (self.chg_price - self.chg_price_mean)*(-1) - self.fee)*self.chg_makereal
        self.chg_posi_var[:1] = 1
        self.chg_posi_entry_cover[:1] = -1
    ...
    def step(self, action):
        ...
        if self.step_st+self.obs_len+self.step_len >= len(self.price):
            ...
            if current_mkt_position != 0:
                ...
                self.chg_posi_entry_cover[:1] = -2
                self.chg_makereal[:1] = 1
                self.chg_reward[:] = ((self.chg_price - self.chg_price_mean)*(
                    current_mkt_position) - abs(current_mkt_position)*self.fee)*self.chg_makereal
        ...
        if action == 1 and self.max_position > current_mkt_position >= 0:
            ...
            self._long(open_posi, enter_price,
                       current_mkt_position, current_price_mean)

        elif action == 2 and -self.max_position < current_mkt_position <= 0:
            open_posi = (current_mkt_position == 0)
            self._short(open_posi, enter_price,
                        current_mkt_position, current_price_mean)
        ...
        self.chg_reward_fluctuant[:] = (
            self.chg_price - self.chg_price_mean)*self.chg_posi - np.abs(self.chg_posi)*self.fee
    ...
    def _plot_trading(self):
        ...
        self.price_plot = self.ax.plot(
            price_x, self.price[:self.step_st+self.obs_len], c=(0, 0.68, 0.95, 0.9), zorder=1)
        ...
        self.fluc_reward_plot_p = self.ax2.fill_between(price_x, 0, self.reward_fluctuant_arr[:self.step_st+self.obs_len],
                                                        where=self.reward_fluctuant_arr[:self.step_st +
                                                                                        self.obs_len] >= 0,
    ...
    def render(self, save=False):
        if self.render_on == 0:
            ...
            self.fig.suptitle('%s' % pd.to_datetime(
                self.df_sample['datetime'], unit="s").iloc[0].date(), fontsize=14, fontweight='bold')
            ...
            self.features_color = [
                c.rgb+(0.9,) for c in Color('yellow').range_to(Color('cyan'), self.feature_len)]
            # fig, ax = plt.subplots()
            self._plot_trading()
